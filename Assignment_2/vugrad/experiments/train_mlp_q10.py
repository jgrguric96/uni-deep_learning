...
for two_hidden_sizes in range(3):
    ...
    class MLP(vg.Module):
        ...
        def __init__(self, input_size, output_size, hidden_mult=4):
            ...
            if two_hidden_sizes == 1:
                hidden_size = int(2 * input_size)
            elif two_hidden_sizes == 2:
                hidden_size = int(0.9 * input_size)
    ...
    if two_hidden_sizes == 0:
        original_hidden_size = val_accuracy
    elif two_hidden_sizes == 1:
        small_hidden_size = val_accuracy
    elif two_hidden_sizes == 2:
        smaller_hidden_size = val_accuracy

fig, ax = plt.subplots()
ax.plot(original_hidden_size, label='Input size * 4')
ax.plot(small_hidden_size, label='Input size * 2')
ax.plot(smaller_hidden_size, label='Input size * 0.9')
ax.set_xlabel('Epoch')
ax.set_ylabel('Validation Accuracy')
ax.set_title("Accuracy between different hidden layer sizes (lr = 0.0001)")
ax.legend()
fig.show()
