...
for two_activations in range(2):
    ...
    class MLP(vg.Module):
        ...
        def forward(self, input):
            ...
            #Sigmoid activation
            if two_activations == 0:
                hidden = vg.sigmoid(hidden)
            #Relu activation
            if two_activations == 1:
    ...
    if two_activations == 0:
        sigm_acc = val_accuracy
    if two_activations == 1:
        relu_acc = val_accuracy

fig, ax = plt.subplots()
ax.plot(sigm_acc, label='Sigmoid Activation')
ax.plot(relu_acc, label='ReLU Activation')
ax.set_xlabel('Epoch')
ax.set_ylabel('Validation Accuracy')
ax.set_title("Accuracy difference between two non-linear activations")
ax.legend()
fig.show()
