...
class ReLU(Op):
    """
    Op for element-wise application of rectified linear unit activation function
    """
    ...
    def backward(context, goutput):
        ...
        # In theory, when relx is 0, then it is non-differentiable
        test = (relux > 0).astype(int)

        return goutput * (relux > 0).astype(float)
